...
// Global click feedback for buttons/tools. Purely visual; it does not change app logic.
document.addEventListener("pointerdown",function(e){
  const el=e.target.closest("button,.nav-item,.tool,.action-card,[role="button"],.card.clickable");
...
/* FINTRACK V12 - robust settings + interaction micro-animations */
(function(){
  function getDB(){
    try { return JSON.parse(localStorage.getItem("fintrack_db") || localStorage.getItem("fintrack") || "{}"); }
    catch(e){ return {}; }
  }
  function putDB(db){
    localStorage.setItem("fintrack_db", JSON.stringify(db));
    localStorage.setItem("fintrack", JSON.stringify(db));
  }
  function currentUser(){
    return localStorage.getItem("fintrack_current_user") || localStorage.getItem("currentUser") || "";
  }
  function saveBudget(){
    const input = document.querySelector("#budgetInput,#monthlyBudget,#budget");
    if(!input) return;
    const raw = String(input.value || "").replace(/[^\d]/g,"");
    const budget = raw ? Number(raw) : 0;
    if(!Number.isFinite(budget) || budget < 0){
      showFTToast("Budget tidak valid");
      return;
    }
    const db=getDB(), u=currentUser();
    if(!u){ showFTToast("Silakan login terlebih dahulu"); return; }

    // Support common schemas used by the previous FinTrack versions.
    db.users = db.users || {};
    if(!db.users[u]) db.users[u] = {username:u};
    db.users[u].budget = budget;
    db.users[u].monthlyBudget = budget;
    db.users[u].settings = Object.assign({}, db.users[u].settings, {monthlyBudget:budget, budget:budget});
    db.settings = db.settings || {};
    db.settings[u] = Object.assign({}, db.settings[u], {monthlyBudget:budget, budget:budget});
    putDB(db);
    localStorage.setItem("fintrack_budget_"+u, String(budget));

    // Update common dashboard labels immediately.
    document.querySelectorAll("[data-budget],[data-monthly-budget]").forEach(el=>{
      el.textContent = "Rp " + budget.toLocaleString("id-ID");
    });
    showFTToast("✓ Pengaturan berhasil disimpan");
  }
  function showFTToast(msg){
    let t=document.getElementById("ftToast");
    if(!t){t=document.createElement("div");t.id="ftToast";document.body.appendChild(t);}
    t.textContent=msg; t.classList.remove("show"); void t.offsetWidth; t.classList.add("show");
    clearTimeout(window.__ftToastTimer); window.__ftToastTimer=setTimeout(()=>t.classList.remove("show"),2200);
  }
  window.fintrackSaveBudget=saveBudget;

  document.addEventListener("click", function(e){
    const save=e.target.closest("#saveSettings,.save-settings,.saveSettingBtn,[data-action='save-settings']");
    if(save){ e.preventDefault(); e.stopPropagation(); saveBudget(); return; }

    // Universal tap feedback for interactive elements.
    const target=e.target.closest("button,a,[role='button'],.clickable,.nav-item,.tool,.card,.fab");
    if(target){
      target.classList.remove("ft-tap"); void target.offsetWidth; target.classList.add("ft-tap");
      const r=document.createElement("span"); r.className="ft-ripple";
      const rect=target.getBoundingClientRect();
      r.style.left=(e.clientX-rect.left)+"px"; r.style.top=(e.clientY-rect.top)+"px";
      target.appendChild(r); setTimeout(()=>r.remove(),650);
    }
  }, true);

  // Keyboard fallback for settings form.
  document.addEventListener("submit", function(e){
    const f=e.target;
    if(f && (f.id==="settingsForm" || f.matches("[data-form='settings']"))){
      e.preventDefault(); saveBudget();
    }
  }, true);
})();
